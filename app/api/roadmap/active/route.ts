...
import { getActiveRoadmap, getTodaySchedule } from "@/lib/roadmap";
...
export async function GET() {
  try {
    ...
    if (!roadmap) {
      ...
      roadmap = pausedRoadmap || null;
    }

    if (!roadmap) {
      return NextResponse.json({ roadmap: null, today_schedule: null });
    }

    const todaySchedule =
      roadmap.status === "paused"
        ? null
        : await getTodaySchedule(roadmap.id, supabase);
    ...
    console.error("[ROADMAP/ACTIVE] Error:", err);
