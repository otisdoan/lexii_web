  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(({ data: { user } }) => {
      ...
      activeChannel = supabase
        .channel(`notifications-page-admin-${user.id}`)
        .on(
          'postgres_changes',
          {
            event: '*',
      ...
      <div className="bg-white border border-slate-100 rounded-2xl p-4 sm:p-5">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-slate-900">Thông báo quản trị</h1>
              <p className="text-sm text-slate-500">Giao dịch mới, trạng thái thanh toán và cập nhật hệ thống.</p>
            </div>
          </div>
          <button
            type="button"
